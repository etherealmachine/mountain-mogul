...
//
// Model metadata: the .scad source can publish arbitrary anchor points via
// openscad's echo() function, e.g.
//
...
// coordinates, and emits them at the head of the OBJ as `# slot N x y z`
// comment lines. Downstream consumers (the game's OBJ loader) parse those
// comments back into Mesh.Slots.
package main
...
func main() {
	...
	// Slots from openscad echo() — converted to game coords here so the
	// loader doesn't need to know about the SCAD axis convention.
	if echoPath != "" {
